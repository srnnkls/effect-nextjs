...
- `Next.middleware` validates "is this middleware provided by the layer" through a phantom rest parameter instead of a conditional on the `middleware` parameter, and the `Next` interface's `Middleware` type parameter is unconstrained. Both changes keep middleware types concrete when tag classes carry a `static readonly layer` (the v4 layer convention); the old encoding silently widened them to `TagClassAny`, dropping `provides` from handler requirements. An unprovided middleware now fails with a missing-argument error naming the problem.
